...
class DiscosClient:
    ...
    def fetch_satellite_dimensions(self, norad_id: str) -> dict:
        ...
        if not self.api_token:
            logger.warning("[ESA DISCOS API] API token not configured. Using fallback structural dimensions.")
            return default_specs
            
        try:
            # Query ESA DISCOS objects endpoint using the NORAD ID
            url = f"https://discos.esoc.esa.int/api/v1/objects?filter=satnoeq({norad_id})&include=initial_orbit,reentry"
            ...
            if r.status_code == 200:
                ...
                if "data" in data and len(data["data"]) > 0:
                    ...
                    logger.info(f"[ESA DISCOS API] Successfully retrieved structural specs for NORAD {norad_id}.")
                    return {
                        "norad_id": norad_id,
            ...
            logger.error(f"[ESA DISCOS API] Query failed for NORAD {norad_id}: {e}")
